...
//3.31 1111111111111111111111111111
#include <iostream>
...
void left_turn();
void view_correction(int rabbit);

void findColour(Mat img, int cat);
...
//        createTrackbar("Val max", "Trackbars", &vmax, 255);
    ...
    while(1)
    {
        ...
        findColour(img, dog); //识别!!

        if(turn_flag == 0 && colour_flag == 5){
...
void view_correction(int rabbit){
    cout << "raaaaaaaaaaaaabbit" << rabbit << endl;
    if(rabbit > 10){
        serialPrintf(robot, "#Baffrr %03d %03d %03d %03d", 55, 55, 35, 35);
        delay(200);
        stop(); 
        ...
        serialPrintf(robot, "#Barrff %03d %03d %03d %03d", 35, 35, 55, 55);
        delay(200);
        stop();
    }
}
